...
const WAKEUP_SESSION = 0
const WAKEUP_ENTRY = 1
...
wakeup.topic = function (topic) {
  const state = alloc()
  const start = state.start
  UINT.encode(state, TL_WAKEUP)
  c.fixed32.encode(state, topic)
  UINT.encode(state, WAKEUP_SESSION)
  return state.buffer.subarray(start, state.start)
}

wakeup.entry = function (topic, clock) {
  const state = alloc()
  const start = state.start
  UINT.encode(state, TL_WAKEUP)
  c.fixed32.encode(state, topic)
  UINT.encode(state, WAKEUP_ENTRY)
  UINT.encode(state, clock)
  return state.buffer.subarray(start, state.start)
}

wakeup.entryEnd = function (topic, clock) {
  const state = alloc()
  const start = state.start
  UINT.encode(state, TL_WAKEUP)
  c.fixed32.encode(state, topic)
  UINT.encode(state, WAKEUP_ENTRY + 1)
}

module.exports = { store, core, wakeup }
