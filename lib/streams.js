...
function createDiscoveryKeyStream (db, view, namespace) {
  ...
  ite._readableState.map = (data) => mapAllDiscoveryKeys(data, namespace)
  return ite
}
...
function mapAllDiscoveryKeys (data, namespace) {
  return namespace ? data.value : store.discoveryKey(data.key)
}
