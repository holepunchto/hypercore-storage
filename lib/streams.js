...
function createDiscoveryKeyStream(db, view, namespace) {
  const start = namespace
    ? store.coreByAliasStart(namespace)
    : store.coreStart()
  const end = namespace ? store.coreByAliasEnd(namespace) : store.coreEnd()

  const ite = view.iterator(db, start, end, false)

  ite._readableState.map = namespace
    ? mapNamespaceDiscoveryKeys
    : mapAllDiscoveryKeys
  return ite
}
...
function createUserDataStream(
  ...
  const e =
    lt === null
      ? core.userDataEnd(ptr.dataPointer)
      : core.userData(ptr.dataPointer, lt)
  const ite = view.iterator(db, s, e, false)
...
function createLocalStream(
  ...
  const e =
    lt === null
      ? core.localEnd(ptr.dataPointer)
      : core.local(ptr.dataPointer, lt)
  const ite = view.iterator(db, s, e, false)
