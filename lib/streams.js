...
const { core, store, wakeup } = require('./keys.js')
...
function createWakeupStream(topic, db, { lt = null, reverse = false } = {}) {
  const s = wakeup.entry(topic, 0)
  const e = lt === null ? wakeup.entryEnd(topic) : wakeup.entry(topic, lt)
  const ite = db.iterator({ gte: s, lt: e, reverse })

  ite._readableState.map = mapWakeup
  return ite
}
...
function mapWakeup(data) {
  return WAKEUP_ENTRY.decode({
    start: 0,
