...
function createDiscoveryKeyStream (db, view, namespace) {
  ...
  if (namespace) ite._readableState.map = mapNamespaceDiscoveryKeys
  else ite._readableState.map = mapAllDiscoveryKeys
  return ite
}
...
function mapAllDiscoveryKeys (data) {
  return { discoveryKey: store.discoveryKey(data.key) }
}

function mapNamespaceDiscoveryKeys (data) {
  return { discoveryKey: data.value }
}
