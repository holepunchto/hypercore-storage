...
class CoreTX {
  constructor(core, db, view, changes, cache) {
  ...
  flush() {
    ...
    if (this.view) {
      ...
      if (this.cache && this._treeAltered) this.cache.invalidate()
      return Promise.resolve(false)
    }

    return View.flush(changes, this.db).then((result) => {
      if (this.cache && this._treeAltered) this.cache.invalidate()
      return result
    })
  }
...
class CoreRX {
  constructor(core, db, view, cache) {
    this.core = core
  ...
  async getTreeNode(index) {
    ...
    const key = `treeNode/${data}:${index}`

    const boundFetch = async () =>
      decode(CORE_TREE_NODE, await this.view.get(this.read, core.tree(data, index)))

    return this.cache.get(key, boundFetch)
  }
