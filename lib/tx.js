...
class CoreTX {
  ...
  setAuth(auth) {
    this.changes.push([
      core.auth(this.core.corePointer),
      encode(CORE_AUTH, auth),
      null
    ])
  }
  ...
  setSessions(sessions) {
    this.changes.push([
      core.sessions(this.core.corePointer),
      encode(CORE_SESSIONS, sessions),
      null
    ])
  }
  ...
  setHead(head) {
    this.changes.push([
      core.head(this.core.dataPointer),
      encode(CORE_HEAD, head),
      null
    ])
  }
  ...
  setDependency(dep) {
    this.changes.push([
      core.dependency(this.core.dataPointer),
      encode(CORE_DEPENDENCY, dep),
      null
    ])
  }
  ...
  setHints(hints) {
    this.changes.push([
      core.hints(this.core.dataPointer),
      encode(CORE_HINTS, hints),
      null
    ])
  }
  ...
  putBitfieldPage(index, data) {
    this.changes.push([
      core.bitfield(this.core.dataPointer, index, 0),
      data,
      null
    ])
  }
  ...
  deleteLocalRange(start, end) {
    this.changes.push([
      ...
      null,
        ...
        ? core.localEnd(this.core.dataPointer)
        : core.local(this.core.dataPointer, end)
    ])
  }
...
class CoreRX {
  ...
  async getAuth() {
    return await decode(
      CORE_AUTH,
      await this.view.get(this.read, core.auth(this.core.corePointer))
    )
  }
  ...
  async getHead() {
    return await decode(
      CORE_HEAD,
      await this.view.get(this.read, core.head(this.core.dataPointer))
    )
  }
  ...
  async getTreeNode(index) {
    ...
    return decode(
      CORE_TREE_NODE,
      await this.view.get(this.read, core.tree(data, index))
    )
  }
...
class CorestoreTX {
  ...
  putCore(discoveryKey, ptr) {
    this.changes.push([
      store.core(discoveryKey),
      encode(CORESTORE_CORE, ptr),
      null
    ])
  }
...
class CorestoreRX {
  ...
  async getCore(discoveryKey) {
    return decode(
      CORESTORE_CORE,
      await this.view.get(this.read, store.core(discoveryKey))
    )
  }
