...
const CORE_BATCHES = schema.getEncoding('@core/batches')
const CORE_HEAD = schema.getEncoding('@core/head')
const CORE_TREE_NODE = schema.getEncoding('@core/tree-node')
const CORE_DEPENDENCIES = schema.getEncoding('@core/dependencies')
...
class CoreTX {
  ...
  setAuth (auth) {
    this.updates.put(core.auth(this.core.corePointer), encode(CORE_AUTH, auth))
  }

  setBatches (batches) {
    this.updates.put(core.batches(this.core.corePointer), encode(CORE_BATCHES, batches))
  }

  setHead (head) {
    this.updates.put(core.head(this.core.dataPointer), encode(CORE_HEAD, head))
  }

  setDependencies (deps) {
    this.updates.put(core.dependencies(this.core.dataPointer), encode(CORE_DEPENDENCIES, deps))
  }

  setHints (hints) {
    this.updates.put(core.dependencies(this.core.dataPointer), encode(CORE_HINTS, hints))
  }
...
class CoreRX {
  ...
  async getAuth () {
    return await decode(CORE_AUTH, await this.updates.get(this.read, core.auth(this.core.corePointer)))
  }

  async getBatches () {
    return await decode(CORE_BATCHES, await this.updates.get(this.read, core.batches(this.core.corePointer)))
  }

  async getHead () {
    return await decode(CORE_HEAD, await this.updates.get(this.read, core.head(this.core.dataPointer)))
  }

  async getDependencies () {
    return await decode(CORE_DEPENDENCIES, await this.updates.get(this.read, core.dependencies(this.core.dataPointer)))
  }

  async getHints () {
    return await decode(CORE_HINTS, await this.updates.get(this.read, core.hints(this.core.dataPointer)))
  }
...
class CorestoreRX {
  ...
  getCoreByAlias (alias) {
    return this.updates.get(this.read, store.coreByAlias(alias))
  }
...
class CorestoreTX {
  ...
  putCoreByAlias (alias, discoveryKey) {
    this.updates.put(store.coreByAlias(alias), discoveryKey)
  }
