...
const { store, core, wakeup } = require('./keys.js')
...
const WAKEUP_SESSION = schema.getEncoding('@wakeup/session')
const WAKEUP_ENTRY = schema.getEncoding('@wakeup/entry')
...
class CorestoreTX {
  ...
  putWakeupSession(topic, session) {
    this.changes.push([wakeup.topic(topic), encode(WAKEUP_SESSION, session), null])
  }

  deleteWakeupSession(topic) {
    this.changes.push([wakeup.topic(topic), null, null])
  }
...
class CorestoreRX {
  ...
  async getWakeupSession(topic) {
    const a = await this.view.get(this.read, wakeup.topic(topic))
    return decode(WAKEUP_SESSION, a)
  }
...
class WakeupTX {
  constructor(topic, db) {
    this.topic = topic
    this.tx = db.write({ autoDestroy: true })
  }

  updateSession(session) {
    this.tx.tryPut(wakeup.topic(this.topic), encode(WAKEUP_SESSION, session))
  }

  putWakeup(clock, entry) {
    this.tx.tryPut(wakeup.entry(this.topic, clock), encode(WAKEUP_ENTRY, entry))
  }

  deleteWakeup(clock) {
    this.tx.tryDelete(wakeup.entry(this.topic, clock))
  }

  deleteWakeupRange(start, end) {
    this.tx.tryDeleteRange(
      wakeup.entry(this.topic, start),
      wakeup.entry(this.topic, end === -1 ? Infinity : end)
    )
  }
...
module.exports = { CorestoreTX, CorestoreRX, CoreTX, CoreRX, WakeupTX }
