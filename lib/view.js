...
class View {
  ...
  iterator (db, start, end, reverse) {
    ...
    if (this.overlay === null || this.overlay.indexed !== this.indexed) {
      const changes = this.map === null ? [] : [...this.map.values()]
      ...
      this.overlay = { indexed: this.indexed, changes, ranges, reverse }
    }

    if (this.overlay.reverse !== reverse) {
      return new OverlayStream(
        stream,
        start,
        end,
        reverse,
        reverseArray(this.overlay.changes),
        reverseArray(this.overlay.ranges)
      )
    }
