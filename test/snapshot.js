...
test('read and write hypercore blocks from snapshot', async (t) => {
...

  {
    ...
    t.is(
      b4a.toString(res[2]),
      'block2',
      'sanity check: does exist in non-snapshot core'
    )
  }
})
...
test('snapshots from atomized core do not get updated', async (t) => {
  ...
  t.alike(
    await readBlocks(atomInitSnap, 2),
    [null, null],
    'init atom snap did not change'
  )
  t.alike(
    await readBlocks(initSnap, 2),
    [null, null],
    'init snap did not change'
  )
  ...
  const expectedOverride = [
    b4a.from('override-block0'),
    b4a.from('override-block1')
  ]
  t.alike(await readBlocks(atomCore, 2), expectedOverride, 'sanity check')

  t.alike(
    await readBlocks(atomPostWriteSnap, 2),
    expected,
    'post-write atom snap did not change'
  )
  t.alike(
    await readBlocks(atomInitSnap, 2),
    [null, null],
...
test('snapshots immutable (all operations)', async (t) => {
  ...
  t.not(
    ...
    [null, null, null],
    'sanity check (core itself got updated'
  )
  t.alike(
  ...
  t.alike(
    await getBitfieldPages(snap, 2),
    [null, null],
    'bitfield pages unchanged'
  )
})
...
test('snapshot deps are immut', async function (t) {
  ...
  t.alike(snap.core.dependencies, [
    { dataPointer: core.core.dataPointer, length: 5 }
  ])
  t.alike(session.core.dependencies, [
    { dataPointer: core.core.dataPointer, length: 6 }
  ])
